...
public class InitializationConfigScritp : MonoBehaviour {
	...
	IEnumerator LoadRes(){
		yield return new WaitForEndOfFrame();//加上这么一句就可以先显示加载画面然后再进行加载  // 总音乐
...
*/
        ...
        if (SocketEventHandle.getInstance().CreateClubCallBack != null)
        {
            SocketEventHandle.getInstance().CreateClubCallBack = null;
        }
