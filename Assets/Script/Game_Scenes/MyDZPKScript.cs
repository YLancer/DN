...
//		}
	...
	private Vector3 getHandCardPosition(int i, int cardIndex){
		Vector3 vec = new Vector3 (0,0,0);
		switch (i) {
		case 0:
			if (cardIndex == 0) {
				//vec = new Vector3 (20, -196, 0);
				vec = new Vector3 (-106, -205, 0);
			} else {
			...
			break;
		case 1:
			vec = new Vector3 (-422, -176, 0);
			break;
		case 2:
			vec = new Vector3 (-515, 20, 0);
			break;
		case 3:
			vec = new Vector3 (-395, 204, 0);
			break;
		case 4:
			vec = new Vector3 (406, 188, 0);
			break;
		case 5:
			vec = new Vector3 (530, -15, 0);
			break;
		default:
			break;
		}
...
//		}
	...
	public void onlineNotice (ClientResponse  response)
	{
		...
		case DirectionEnum.TopRight:
			...
			break;
		case DirectionEnum.Right:
			playerItems [5].GetComponent<PlayerItemScript> ().setPlayerOnline ();
			break;
		}
	}
...
//		text_dichi.text = dichi_dzpk.ToString ();
	...
	private int getIndexByDir (string dir)
	{
		...
		case DirectionEnum.Bottom: //下
			result = 0;
			break;
		case DirectionEnum.LeftBottom: //下左
			result = 1;
			break;
	...
	*/
	...
	private string getDirection (int avatarIndex)
	{
		...
		//从自己开始计算，下一位的索引
		if (GlobalDataScript.roomVo.gameType == 4) {//德州扑克位置是顺时针
			for (int i = 0; i < 6; i++) {
				myselfIndex++;
				if (myselfIndex >= 6) {
					myselfIndex = 0;
...
//		SetTip ();
	...
	public void setRoomRemark ()
	{
		...
		if (GlobalDataScript.goldType) {
			...
			roomInfo += "房间号:" + roomvo.roomId + "/n";
			roomInfo += "局数:" + GlobalDataScript.roomVo.roundNumber+"/n";
		}
...
//					playerItems [getIndexByDir (getDirection (getIndex (uuid)))].showDZPKSuccess ();
					playerItems [getIndexByDir (getDirection (getIndex (uuid)))].setScoreText(score);
				}
...
//			}
	...
	public void DZPK_faPaiResponse(ClientResponse response){
		FaPai_DZPKVO favo = JsonMapper.ToObject<FaPai_DZPKVO> (response.message);
		List<int> deskCard = favo.deskCard;
		int count = handerCardList [6].Count;
...
//		}
	...
	private Vector3 getPlayerPosition (int i){
		...
		switch (i) {
		case 0:
			vec = new Vector3 (-108, -218, 0);
			break;
		case 1:
			vec = new Vector3 (-425, -191, 0);
			break;
		case 2:
			vec = new Vector3 (-513, 11.4f, 0);
			break;
		case 3:
			vec = new Vector3 (-400, 209, 0);
			break;
		case 4:
			vec = new Vector3 (412, 199, 0);
			break;
		case 5:
			vec = new Vector3 (533, -30, 0);
			break;
		default:
			break;
		}
	...
	private Vector3 getZhuCoinPosition (int i){
		...
		switch (i) {
		case 0:
			vec = new Vector3 (-38, -119, 0);
			break;
		case 1:
			vec = new Vector3 (-343, -99, 0);
			break;
		case 2:
			vec = new Vector3 (-422, 97, 0);
			break;
		case 3:
			vec = new Vector3 (-306, 280, 0);
			break;
		case 4:
			vec = new Vector3 (341, 264, 0);
			break;
		case 5:
			vec = new Vector3 (445, 52, 0);
			break;
		default:
			break;
		}
...
//		}
    ...
    */
	public void openGameSettingDialog ()
	{
		...
		SettingScript SettingScript = panelCreateDialog.GetComponent<SettingScript> ();
		if (canClickButtonFlag) {
			SettingScript.canClickButtonFlag = canClickButtonFlag;
			SettingScript.jiesanBtn.GetComponentInChildren<Text> ().text = "申请解散房间";
			SettingScript.type = 2;            
		} else {
			if (0 == getMyIndexFromList ()) { //我是房主
				SettingScript.canClickButtonFlag = canClickButtonFlag;
				SettingScript.jiesanBtn.GetComponentInChildren<Text> ().text = "解散房间";
				SettingScript.type = 3;
				SettingScript.dialog_fanhui = dialog_fanhui;
				dialog_fanhui_text.text = "亲，确定要解散房间吗?";
			} else {
				SettingScript.canClickButtonFlag = canClickButtonFlag;
				SettingScript.jiesanBtn.GetComponentInChildren<Text> ().text = "离开房间";
				SettingScript.type = 3;
				SettingScript.dialog_fanhui = dialog_fanhui;
